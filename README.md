...
This project is a **Java Spring Boot application** which integrates any seasonal ticket (e.g. the "**Deutschlandticket**" or any **Semester ticket**) into **Deutsche Bahn connection searches**. It helps users to determine which additional tickets they need to purchase based on their selected seasonal tickets.
...
#### Upcoming
The additional search-modes `DAILY` and `PERIODIC` are being worked on and soon to be added.
...
3. Add the new ticket to the `Ticket` Enum.
4. Add an case for the new ticket in `getSeasonalTickets()` in the `SearchRequest` class.

## Known issues (Work in Progress)
This project is very much Work in Progress. There are a lot of features in need of improvement, or even to be added.

Here's a list of all the known issues or missing features so far:
- **Missing search-modes**: There are two missing search-modes: `DAILY` and `PERIODIC`, they are being worked on. Until the rate limiting isn't implemented there are too many cases where the error occurs on these modes. 
- **Exception handler**: Some exceptions (e.g. `TrainNotFoundException`) arent handled via the `GlobalExceptionHandler` yet, that needs to be changed.
- **Rate limiting**: When too many connections are requested by the `ExternalApiClient` it receives a `429 TOO_MANY_REQUESTS`. To fix this issue rate limiting need to be implemented aswell as an better retry mechanism.
- **Missing documentation**: Some classes are in need of better or additional documentation.
- **Missing tests**: Tests need to be added for different scenarios.
...
At [Known issues](#known-issues--work-in-progress-) you can find some issues which need to be worked on, maybe you are the one who can fix them. If you have some new features on your mind feel free to make a pull request aswell.


## API Reference

When running, the application acts as a **REST-API** with the following endpoints:
1. **GET /** 
- **Description**: Returns a friendly welcome message to confirm the API is up and running.
